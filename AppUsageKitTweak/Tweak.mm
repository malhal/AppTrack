...
static void replaced_SBApplication_willDeactivateForEventsOnly(SBApplication* self, SEL sel, BOOL eventsOnly){
    ...
    AUKTManager* a = [AUKTManager sharedManager];
...
(*original_SBApplicationController_sendInstalledAppsDidChangeNotification$removed$modified$)(self,sel,added,removed,modified);
    ...
    if(_first){
        ...
        NSFetchRequest* countRequest = [NSFetchRequest fetchRequestWithEntityName:[AUKTLifecycle entityName]];
    ...
    BOOL (^insertLifecycle)(NSString* bundleIdentifier, AUKLifecycleChangeType changeType, NSError** error) = ^BOOL(NSString* bundleIdentifier, AUKLifecycleChangeType changeType, NSError** error){
        ...
        if(!bundleVersion){
            if(changeType != AUKLifecycleChangeTypeRemoved){
            ...
            NSFetchRequest* fetchRequest = [NSFetchRequest fetchRequestWithEntityName:[AUKTLifecycle entityName]];
    ...
    for(NSString* bundleIdentifier in added){
        if(!insertLifecycle(bundleIdentifier, AUKLifecycleChangeTypeAdded, &error)){
            break;
        }
    }
    ...
    if(!error){
        for(NSString* bundleIdentifier in modified){
            if(!insertLifecycle(bundleIdentifier, AUKLifecycleChangeTypeModified, &error)){
                break;
            }
        }
    }
    ...
    if(!error){
        for(NSString* bundleIdentifier in removed){
            if(!insertLifecycle(bundleIdentifier, AUKLifecycleChangeTypeRemoved, &error)){
                break;
            }
        }
    }
