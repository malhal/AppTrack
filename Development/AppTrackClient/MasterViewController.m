...
@property (nonatomic) NSMutableArray<ATObject*> *objects;
@property ATStore* store;
...
-(void)reload{
    ...
    ATUsage* firstObject = (ATUsage*)self.objects.firstObject;
    if(firstObject){
        predicate = [NSPredicate predicateWithFormat:@"startDate > %@", firstObject.startDate];
    }
    ATObjectType* objectType = [ATObjectType objectTypeForIdentifier:ATObjectTypeIdentifierUsage];
...
/*
    ...
    ATObjectType* objectType = [ATObjectType objectTypeForIdentifier:ATObjectTypeIdentifierLifecycle];
    NSSortDescriptor* sort = [[NSSortDescriptor alloc] initWithKey:@"date" ascending:NO];
*/
    
    ATQuery* query = [[ATQuery alloc] initWithObjectType:objectType predicate:predicate limit:ATObjectQueryNoLimit sortDescriptors:@[sort] resultsHandler:^(ATQuery * _Nonnull query, NSArray<__kindof ATObject *> * _Nullable results, NSError * _Nullable error) {
        if(error){
            NSLog(@"error %@", error);
            return;
        }
...
-(void)queryApps{
    ATObjectType* appObjectType = [ATObjectType objectTypeForIdentifier:ATObjectTypeIdentifierApp];
    ATQuery* query = [[ATQuery alloc] initWithObjectType:appObjectType predicate:nil limit:ATObjectQueryNoLimit sortDescriptors:nil resultsHandler:^(ATQuery * _Nonnull query, NSArray<__kindof ATObject *> * _Nullable results, NSError * _Nullable error) {
        if(error){
            NSLog(@"error %@", error);
            return;
        }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"showDetail"]) {
        ...
        ATObject *object = self.objects[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ATUsage *object = (ATUsage *)self.objects[indexPath.row];
